...
def run_ghidra_import(ghidra_path, filename, languageId, offset):
    cmd = ghidra_path + '/support/analyzeHeadless workspace/' + filename + '/ghidra/' + str(offset) + '/' + filename + ' ' + filename + ' -import workspace/' + filename + '/out/* -recursive -noanalysis -processor "' + languageId + '" -loader BinaryLoader'
    subprocess.check_output(cmd, shell=True, text=True, stderr=subprocess.DEVNULL)
